...
export const verifyOTP = async (req, res) => {
    ...
    try {
        ...
        if (!user.resetOTP) {
            return res
                .status(400)
                .json({ message: 'No OTP set for this user' });
        }
        console.log(
            'Stored OTP:',
            user.resetOTP,
            'Type:',
            typeof user.resetOTP
        );
...
export const sendEmailVerificationOTP = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res
                .status(500)
                .json({ message: 'Failed to send email verification OTP.' });
        }
    } catch (error) {
        console.error('Error sending email verification OTP:', error);
        return res
...
export const verifyEmailOTP = async (req, res) => {
    ...
    try {
        ...
        if (!user.emailVerificationOTP) {
            return res.status(400).json({
                message: 'No email verification OTP set for this user',
            });
        }
